...
const interactiveApps = [
    { title: 'Understanding Layer — Agent Trace Viewer', path: '/apps/understanding-layer-demo.html', description: 'Interactive explorer for a real gpt-4.1 SWE-agent trace (46 events from MIRAGE-Bench). Phase-segmented timeline with type/actor filters, full-text search, a trace-analytics dashboard, per-event commentary, causal-chain highlighting, error-chain grouping, a guided tour, keyboard navigation, and dark/light themes — all framework-free static JS.', category: 'AI/ML', featured: true },
    ...
    { title: 'Mech Interp Viz', path: '/apps/mechmap.html', description: 'Interactive mechanistic interpretability visualization for transformer models. Annotate attention heads and MLP blocks, discover circuits, and export research findings. Built with Next.js + circuit discovery UI.', category: 'AI/ML', featured: true },
    { title: 'Attn Flow', path: '/apps/attn_flow_web.html', description: 'Browser-native attention flow visualizer — watch probability mass move through GPT-2\'s residual stream in real time. Particle simulation shows token competition at each layer, color-coded by token identity. Ported from a C terminal tool; no install required.', category: 'AI/ML', featured: true },
    { title: 'The Frequency Prior Explorer', path: '/apps/frequency-prior-explorer/', description: 'Interactive companion to the Frequency Prior research series. Walk the Resolution Ladder (attention heads → directions → SAE features), run simulated causal interventions built from real measured GPT-2 data, trace per-country logit-lens trajectories, and browse verified Paper 5 findings with plain-language explainers. React + Recharts.', category: 'AI/ML', featured: true },
    ...
    { title: 'Linear Algebra Learning Lab', path: '/apps/linear-calculator.html', description: 'Executable linear algebra: edit matrices, inspect live invariants, visualize transformations, and launch examples from a guided concept library.', category: 'Math', featured: true },
    ...
    { title: 'Flow Writer', path: '/apps/flow-writer-v2/', description: 'Narrative writing tool with story branching, timeline visualization, sprint engine, voice-to-text, AI assistant, and universe dashboard.', category: 'Tools', featured: true },
